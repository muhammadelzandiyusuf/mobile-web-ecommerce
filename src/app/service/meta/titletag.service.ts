...
export class TitleTagService {
  ...
  public setSocialMediaTags(url: string, title: string, description: string, image: string, text: string, site: string): void {
    var imageUrl = `https://asset.kitchenart.id/images/referral/share/${image}`;
    var tags = [
      ...
      new MetaTag(this.twitterTitleMeta, text, false),
      new MetaTag(this.siteNameMeta, site, true)
    ];
    this.setTags(tags);
  }
