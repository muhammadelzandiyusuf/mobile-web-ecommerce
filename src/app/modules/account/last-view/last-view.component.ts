...
@Component({
  ...
  styleUrls: ['./last-view.component.css']
})
export class LastViewComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.getMeta()
    this.titleService.setTitle('KitchenArt - Last View');
  }
  ...
  initialiseInvites() {
    ...
    if(this.token){
      this.getDataLastView(this.token)
    }
  }
  ...
  getDataLastView(token :any){
    this.wishlistService.getProductLastView(token)
    .subscribe(lastview => {
      this.products = lastview['kitchenart']['results']
