...
@Component({
  ...
  styleUrls: ['./shipping-address-list.component.css']
})
export class ShippingAddressListComponent implements OnInit, OnDestroy {
  ...
  updateAddres(id: any): void {
    this.router.navigate(['shipping-address/', id]);
  }
  ...
  deleteAddress(id: any, token: any): void {
    this.cekShipping = true
    this.shippingAddressService.postDelete(id, token)
    .subscribe((shipping: any) => {
