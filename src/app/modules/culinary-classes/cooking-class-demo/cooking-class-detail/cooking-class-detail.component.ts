...
@Component({
  selector: 'app-cooking-class-detail',
  templateUrl: './cooking-class-detail.component.html',
  styleUrls: ['./cooking-class-detail.component.css']
})
export class CookingClassDetailComponent implements OnInit, OnDestroy {
  ...
  getDetailCookingClass(url: any) {
    ...
    .subscribe(exhibition => {
      ...
      this.countParticipant = this.exhibitions['count_participant']
      this.quota = this.exhibitions['quota']

      this.link = this.exhibitions['web_domain'] + '/cooking_class_demo/' + url
      this.media = this.imageDomain + '/' + this.bannerPath + '/' + this.bannerImage;

      let i = 1;
      if(this.photoImage.length > 0){
  ...
  getDetailShowroom(link: any): void {
  ...
  formSignUp(){
    this.router.navigate(['cooking_class_demo/registration/', this.param]);
  }
